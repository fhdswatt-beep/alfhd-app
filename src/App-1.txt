  ...
  async function runTest() {
    ...
    try {
      const res = await fetch(AI_REPLY_FUNCTION_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${SUPABASE_KEY}`, 'apikey': SUPABASE_KEY },
        body: JSON.stringify({ test_mode: true, question: testQ.trim() }),
      });
      const data = await res.json();
      setTestResult(data);
  ...
  return (
    <div style={{ padding: 16, maxWidth: 960, margin: '0 auto', direction: 'rtl' }}>
      ...
      {tab === 'dash' && (<>
        ...
        {/* اختبار مباشر — جرب سؤال قبل ما تفعّله على الزبائن */}
        <div style={box}>
          <div style={{ ...label, marginBottom: 8 }}>اختبار مباشر (بدون إرسال للزبون)</div>
          <input style={inp} placeholder="اكتب سؤال زبون تجريبي — مثلاً: ماليبو موجود؟" value={testQ} onChange={(e) => setTestQ(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') runTest(); }} />
          <button onClick={runTest} disabled={testing || !testQ.trim()} style={{ marginTop: 9, padding: '8px 18px', borderRadius: 10, border: 'none', background: testing ? '#3A4658' : '#2AABEE', color: '#fff', fontWeight: 700, fontSize: 12.5, cursor: 'pointer', fontFamily: 'inherit' }}>
            {testing ? 'جاري الاختبار...' : 'جرّب الرد'}
          </button>
          {testResult && (
            <div style={{ marginTop: 11, background: '#0E1621', border: '1px solid #222C42', borderRadius: 10, padding: 11 }}>
              {testResult.error && <div style={{ fontSize: 12, color: '#F25050' }}>خطأ: {String(testResult.error)}</div>}
              {testResult.escalated && <div style={{ fontSize: 12.5, color: '#F59E0B', fontWeight: 700 }}>⚠ صعّده لموظف — السبب: {testResult.reason}</div>}
              {testResult.reply && <div style={{ fontSize: 12.5, color: '#F5F5F5', lineHeight: 1.7 }}><b style={{ color: '#546880' }}>الرد:</b> {testResult.reply}</div>}
              {testResult.would_send_image && (
                <div style={{ marginTop: 8 }}>
                  <div style={{ fontSize: 11.5, color: '#22C55E', marginBottom: 5 }}>✓ راح يرسل صورة المنتج وياه</div>
                  <img src={testResult.would_send_image} alt="" style={{ maxWidth: 150, borderRadius: 8 }} onError={(e) => { e.target.style.display = 'none'; }} />
                </div>
              )}
              {Array.isArray(testResult.tools_called) && testResult.tools_called.length > 0 && (
                <details style={{ marginTop: 9 }}>
                  <summary style={{ fontSize: 11.5, color: '#2AABEE', cursor: 'pointer', fontWeight: 700 }}>على شنو اعتمد؟ ({testResult.tools_called.length} استعلام)</summary>
                  <pre style={{ fontSize: 10.5, color: '#8B98A9', background: '#131B26', padding: 9, borderRadius: 8, overflowX: 'auto', marginTop: 7, direction: 'ltr', textAlign: 'left' }}>
                    {JSON.stringify(testResult.tools_called, null, 1)}
                  </pre>
                </details>
              )}
            </div>
            ...
            return (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', borderBottom: i < filteredProducts.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#546880' }}>
                    {WH_PRODUCT_TYPES.find((t) => t.id === p.type)?.label || p.type}
                    {!p.image_url && <span style={{ color: '#F59E0B' }}> · ⚠ بدون صورة</span>}
                  </div>
                </div>
...
function WhProducts({ products, setProducts, cars, setCars, sbI, sbU, sbD }) {
  ...
  const [form,setForm]=useState({car_name:'',type:'mother_dosah',quantity:0,cost_iqd:0,price_iqd:0,location:'',notes:'',image_url:'',origin:'both',colors:[]});
  ...
  function openNew(){setEditing(null);setNewCarMode(false);setForm({car_name:'',type:'mother_dosah',quantity:0,cost_iqd:0,price_iqd:0,location:'',branch:'',shelf:'',notes:'',image_url:'',origin:'both',colors:[]});setModal(true);}
  function openEdit(p){
    ...
    setForm({car_name:p.car_name||'',type:p.type||'mother_dosah',quantity:p.quantity||0,cost_iqd:p.cost_iqd||0,price_iqd:p.price_iqd||0,location:loc,branch,shelf,notes:p.notes||'',image_url:p.image_url||'',origin:p.origin||'both',colors:Array.isArray(p.colors)?p.colors:[]});setModal(true);
  }
  ...
  async function save(){
    ...
    const clean={car_name:form.car_name.trim(),type:form.type,quantity:Number(form.quantity)||0,cost_iqd:Number(form.cost_iqd)||0,price_iqd:Number(form.price_iqd)||0,location:locClean,notes:form.notes||'',image_url:form.image_url||'',origin:form.origin||'both',colors:form.colors||[]};
    try{
