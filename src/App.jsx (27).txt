...
function ProductEditPage({ product, onBack, sbI, sbU, sbD, setProducts }) {
  ...
  async function addSeatOption(colorId) {
    try {
      ...
      const r = await sbI('wh_product_seat_options', { color_id: colorId, seats: 5, kashat_type: null, has_box: false, box_required: false, price_iqd: 0, quantity: 0, location: '', ai_available: true, origin: color?.origin || 'both', sort_order: (color?.seatOptions?.length || 0) });
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {c.seatOptions.map(o => {
                      ...
                      <div key={o.id} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${avail ? 'rgba(255,255,255,0.07)' : 'rgba(242,80,80,0.15)'}`, borderRadius: 10, padding: 10 }}>
                        {/* الصف الأول: الركاب + الكشنات + المنشأ */}
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                          ...
                          <input type="number" value={o.quantity ?? 0} onChange={e => updateSeatOption(c.id, o.id, { quantity: e.target.value ? Number(e.target.value) : 0 })} placeholder="الكمية" style={{ ...whInp, width: 90, padding: '6px 8px', fontSize: 12 }} />
...
function WhProducts({ products, setProducts, cars, setCars, sbI, sbU, sbD }) {
  ...
  function openEdit(p){ setEditingFull(p); }
  ...
  if (editingFull) {
    return <ProductEditPage product={editingFull} onBack={() => setEditingFull(null)} sbI={sbI} sbU={sbU} sbD={sbD} setProducts={setProducts} />;
  }
  return (
    <div>
      ...
      <div style={{background:'linear-gradient(145deg,#17212B,#1A2736)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:13,overflow:'hidden'}}>
        ...
        filtered.map((p,i)=>{
          ...
          const avail=p.ai_available!==false;
          return(
            <div key={p.id} style={{padding: '14px 16px',borderBottom:i<filtered.length-1?'1px solid rgba(255,255,255,0.06)':'none'}}>
              {/* الصف الأول: صورة + اسم + نوع */}
              ...
              {/* الصف الثاني: السعر والموقع — يلتف تلقائياً على الموبايل */}
