...
function ConversationsView({ conversations, pages, orders, setConversations, pendingOpenConvId, clearPendingOpenConvId, onCreateOrderFromConv, onOpenOrderDetails, currentUser }) {
 ...
 async function maybeCreateOrderFromAI(convId, messages) {
  ...
  const [gov, area] = address.includes('-') ? address.split('-').map((s) => s.trim()) : [address.trim(), ''];

  try {
   const payload = {
    order_no: String(Date.now()).slice(-6),
    page_id: conv.pageId || null,
    customer_name: recipient || conv.customer || '',
    phone: phoneRaw,
    address,
  ...
  <div style={{ display: 'flex', overflow: 'hidden', direction: 'rtl', background: '#0E1621', width: '100%', height: '100%', }} className="alfhd-conv-fullscreen">
   ...
   >
    ...
    {(currentUser?.role === 'admin' || (currentUser?.permissions || []).includes('ai_manage')) && (
     <button
      onClick={async () => {
        const anyActive = conversations.some((c) => (c.ai_mode || 'active') === 'active');
      ...
      style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, width: '100%', padding: '9px',
       background: conversations.some((c) => (c.ai_mode || 'active') === 'active') ? 'rgba(34,197,94,0.10)' : 'rgba(242,80,80,0.10)',
       border: 'none', borderBottom: `1px solid ${TG_BDR}`,
       color: conversations.some((c) => (c.ai_mode || 'active') === 'active') ? '#22C55E' : '#F25050',
       fontSize: 12, fontWeight: 800, cursor: 'pointer', fontFamily: "'Cairo', sans-serif" }}
     >
      <Bot size={14} />
      {globalAiBusy ? 'جاري التغيير...' : (conversations.some((c) => (c.ai_mode || 'active') === 'active') ? 'الرد الآلي شغّال — اضغط لإيقاف الكل' : 'الرد الآلي متوقف — اضغط لتشغيل الكل')}
     </button>
    )}
    {/* ── قائمة المحادثات ── */}
    ...
    {selectedConv ? ( <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '12px 16px', borderBottom: `1px solid ${TG_BDR}`, background: TG_PANEL, flexShrink: 0,
       ...
       {(currentUser?.role === 'admin' || (currentUser?.permissions || []).includes('ai_manage')) && (
        <button
         onClick={() => {
           const cur = selectedConv.ai_mode || 'active';
         ...
         style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 10px', border: 'none', borderRadius: 20, fontSize: 12, fontWeight: 700, flexShrink: 0, cursor: 'pointer', transition: 'all .15s',
          background: (selectedConv.ai_mode || 'active') === 'active' ? 'rgba(34,197,94,0.14)' : 'rgba(242,80,80,0.12)',
          color: (selectedConv.ai_mode || 'active') === 'active' ? '#22C55E' : '#F25050' }}
        >
         <Bot size={14} />
         <span className="alfhd-ai-btn-label">
          {(selectedConv.ai_mode || 'active') === 'active' ? 'الذكاء فعّال' : 'الذكاء متوقف'}
         </span>
        </button>
       )}
      ...
      {(selectedConv.ai_mode || 'active') === 'active' && (
...
function AIAssistantView({ currentUser }) {
  ...
  const activeConvs = convs.filter((c) => (c.ai_mode || 'active') === 'active').length;
