...
function ProductEditPage({ product, onBack, sbI, sbU, sbD, setProducts }) {
  ...
  async function addSeatOption(colorId) {
    try {
      ...
      const r = await sbI('wh_product_seat_options', { color_id: colorId, seats: 5, kashat_type: null, has_box: false, price_iqd: 0, sort_order: (color?.seatOptions?.length || 0) });
              ...
              <div style={{ marginRight: 84 }}>
                ...
                {c.seatOptions.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {c.seatOptions.map(o => (
                      <div key={o.id} style={{ display: 'flex', gap: 6, alignItems: 'center', background: 'rgba(255,255,255,0.03)', borderRadius: 8, padding: '6px 8px' }}>
                        ...
                        <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11.5, color: '#8FA0B5', cursor: 'pointer' }}>
                          <input type="checkbox" checked={!!o.has_box} onChange={e => updateSeatOption(c.id, o.id, { has_box: e.target.checked })} /> صندوق
                        </label>
                        <input type="number" value={o.price_iqd || ''} onChange={e => updateSeatOption(c.id, o.id, { price_iqd: e.target.value ? Number(e.target.value) : 0 })} placeholder="السعر" style={{ ...whInp, flex: 1, padding: '6px 8px', fontSize: 12 }} />
                        <button onClick={() => deleteSeatOption(c.id, o.id)} style={{ padding: 6, background: 'rgba(242,80,80,0.08)', border: '1px solid rgba(242,80,80,0.18)', borderRadius: 7, color: '#F25050', cursor: 'pointer', flexShrink: 0 }}><Trash2 size={11} /></button>
                      </div>
                    ))}
                  </div>
                )}
