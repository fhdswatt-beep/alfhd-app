                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {c.seatOptions.map(o => {
                      ...
                      <div key={o.id} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${avail ? 'rgba(255,255,255,0.07)' : 'rgba(242,80,80,0.15)'}`, borderRadius: 10, padding: 10 }}>
                        {/* الصف الأول: الركاب + الكشنات + المنشأ + الدفع */}
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                          ...
                          <select value={o.kashat_type || ''} onChange={e => updateSeatOption(c.id, o.id, { kashat_type: e.target.value || null })} style={{ ...whInp, width: 110, padding: '6px 8px', fontSize: 12 }}>
                            <option value="">بدون تحديد كشنات</option>
                            <option value="متصل">كشنات متصل</option>
                            <option value="منفصل">كشنات منفصل</option>
                          </select>
                          <select value={o.origin || 'both'} onChange={e => updateSeatOption(c.id, o.id, { origin: e.target.value })} style={{ ...whInp, width: 100, padding: '6px 8px', fontSize: 12 }}>
                            <option value="both">خليجي وأمريكي</option>
                            <option value="gulf">خليجي</option>
                            <option value="american">أمريكي</option>
                          </select>
                          <select value={o.drivetrain || 'all'} onChange={e => updateSeatOption(c.id, o.id, { drivetrain: e.target.value })} style={{ ...whInp, width: 100, padding: '6px 8px', fontSize: 12 }}>
                            <option value="all">فورويل وتوويل</option>
                            <option value="fourwd">فورويل</option>
                            <option value="twowd">توويل</option>
...
function WhProducts({ products, setProducts, cars, setCars, sbI, sbU, sbD }) {
  ...
  function openEdit(p){ scrollPosRef.current = window.scrollY; setEditingFull(p); }
  ...
  if (editingFull) {
    return <ProductEditPage product={editingFull} onBack={() => { setEditingFull(null); setTimeout(() => window.scrollTo(0, scrollPosRef.current), 0); }} sbI={sbI} sbU={sbU} sbD={sbD} setProducts={setProducts} />;
  }
  return (
    <div>
