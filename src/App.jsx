...
function ConversationsView({ conversations, pages, orders, setOrders, setConversations, pendingOpenConvId, clearPendingOpenConvId, onCreateOrderFromConv, onOpenOrderDetails, currentUser }) {
 ...
 const [convAiBusyId, setConvAiBusyId] = useState(null);
  ...
  <div style={{ display: 'flex', overflow: 'hidden', direction: 'rtl', background: '#0A0E14', width: '100%', height: '100%', }} className="alfhd-conv-fullscreen">
   ...
   >
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px 12px', borderBottom: `1px solid ${TG_BDR}`, flexShrink: 0, }}>
     ...
     {/* ── زر الرد الآلي العام: أخضر=الكل، برتقالي=انتقائي، أحمر=متوقف ── */}
     {(currentUser?.role === 'admin' || (currentUser?.permissions || []).includes('ai_manage')) && (() => {
      const scope = globalAiScope || 'off';
      const isAll = scope === 'all';
      const isSelective = scope === 'selective';
      const color = isAll ? '#22C55E' : isSelective ? '#F59E0B' : '#F25050';
      const bg = isAll ? 'rgba(34,197,94,0.12)' : isSelective ? 'rgba(245,158,11,0.12)' : 'rgba(242,80,80,0.10)';
      const border = isAll ? 'rgba(34,197,94,0.35)' : isSelective ? 'rgba(245,158,11,0.38)' : 'rgba(242,80,80,0.28)';
      const label = isAll ? 'شغال' : isSelective ? `${globalActiveAiCount} شغالة` : 'متوقف';
      return (
       <button
        onClick={async () => {
         if (globalAiBusy || globalAiScope == null) return;
         const turnOnAll = globalAiScope === 'off';
         const msg = turnOnAll
          ? 'تشغيل الرد الآلي لكل المحادثات؟\nمحادثات الموظف (Handoff) تبقى محمية.'
          : (globalAiScope === 'selective'
            ? `إيقاف الرد الآلي عن الجميع؟\nحالياً ${globalActiveAiCount} محادثة شغالة بشكل منفرد.`
            : 'إيقاف الرد الآلي عن الجميع؟');
         ...
         try {
          ...
          setGlobalAiEnabled(scopeNext !== 'off');
          setGlobalActiveAiCount(Number(data.active_count || (turnOnAll ? data.eligible_count || 0 : 0)));
          setConversations?.((prev) => prev.map((c) => ({
           ...c,
           ai_mode: turnOnAll && c.tab !== 'handoff' ? 'active' : (!turnOnAll ? 'paused' : c.ai_mode),
          })));
          setSelectedConv((c) => c ? { ...c, ai_mode: turnOnAll && c.tab !== 'handoff' ? 'active' : (!turnOnAll ? 'paused' : c.ai_mode) } : c);
          setAiToast(turnOnAll ? '✓ تشغيل الرد للجميع' : '✓ إيقاف الرد عن الجميع');
          setTimeout(() => setAiToast(null), 2200);
         } catch (e) { alert('فشل التغيير العام: ' + (e.message || 'خطأ غير معروف')); }
         finally { setGlobalAiBusy(false); }
        }}
        disabled={globalAiBusy || globalAiScope == null}
        title={isSelective ? `تشغيل انتقائي — ${globalActiveAiCount} محادثة شغالة. اضغط لإيقاف الجميع.` : (isAll ? 'الرد شغال لكل المحادثات — اضغط لإيقاف الجميع' : 'الرد متوقف — اضغط لتشغيل الجميع')}
        style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 5, height: 44, padding: '0 10px', borderRadius: 12, flexShrink: 0,
         background: bg, border: `1px solid ${border}`, color, cursor: globalAiBusy ? 'wait' : 'pointer' }}
       >
        <Bot size={17} />
        <span style={{ fontSize: 11, fontWeight: 800, whiteSpace: 'nowrap' }}>{globalAiBusy ? '…' : label}</span>
    ...
    {selectedConv ? ( <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: `1px solid ${TG_BDR}`, background: TG_PANEL, flexShrink: 0,
       ...
       {(currentUser?.role === 'admin' || (currentUser?.permissions || []).includes('ai_manage')) && (
        <button
         onClick={async () => {
           if (selectedConv.tab === 'handoff' || convAiBusyId === selectedConv.id) {
           ...
           try {
             ...
             setConversations?.((prev) => prev.map((c) => (c.id === convId ? { ...c, ai_mode: mode } : c)));
             const scope = data.scope || (next ? 'selective' : 'off');
             setGlobalAiScope(scope);
             setGlobalAiEnabled(scope !== 'off');
             setGlobalActiveAiCount(Number(data.active_count || 0));
             setAiToast(next ? '✓ الرد شغال لهذي المحادثة' : '✓ الرد متوقف لهذي المحادثة');
             setTimeout(() => setAiToast(null), 2200);
           } catch (e) {
             setAiToast('✗ فشل: ' + (e.message || 'خطأ غير معروف'));
             setTimeout(() => setAiToast(null), 4000);
           } finally { setConvAiBusyId(null); }
         }}
         disabled={convAiBusyId === selectedConv.id}
         title={selectedConv.tab === 'handoff' ? 'المحادثة بيد موظف — رجّعها للذكاء أولاً' : (selectedConv.ai_mode === 'active' ? 'إيقاف الرد الآلي عن هذي المحادثة' : 'تشغيل الرد الآلي لهذي المحادثة فقط')}
         style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '8px 8px', border: 'none', borderRadius: 20, fontSize: 12, fontWeight: 700, flexShrink: 0, cursor: selectedConv.tab === 'handoff' ? 'not-allowed' : (convAiBusyId === selectedConv.id ? 'wait' : 'pointer'), transition: 'all .15s',
          background: selectedConv.tab === 'handoff' ? 'rgba(245,158,11,0.12)' : (selectedConv.ai_mode === 'active' ? 'rgba(34,197,94,0.14)' : 'rgba(242,80,80,0.12)'),
          color: selectedConv.tab === 'handoff' ? '#F59E0B' : (selectedConv.ai_mode === 'active' ? '#22C55E' : '#F25050') }}
        >
         <Bot size={14} />
         <span className="alfhd-ai-btn-label">
          {convAiBusyId === selectedConv.id ? '…' : (selectedConv.tab === 'handoff' ? 'موظف' : (selectedConv.ai_mode === 'active' ? 'شغال' : 'متوقف'))}
         </span>
        </button>
       )}
...
function OrdersView({ orders, pages, setOrders, conversations, setConversations, onViewConversation, pendingNewOrderFromConv, clearPendingNewOrderFromConv, currentUser, onContactCustomer, pendingOpenOrderId, clearPendingOpenOrderId, warehouseProducts = [] }) {
  ...
  // قسم الطلبات يستخدم .alfhd-main-area كسكرول رئيسي. نحفظ مكان المستخدم ونرجعه مثل المخزن.
  const ordersScrollPosRef = React.useRef(0);
  const ORDERS_SCROLL_KEY = 'alfhd-orders-scroll-v1';
  function rememberOrdersScroll() {
    const mainEl = document.querySelector('.alfhd-main-area');
    const pos = mainEl ? mainEl.scrollTop : window.scrollY;
    ordersScrollPosRef.current = pos;
    try { sessionStorage.setItem(ORDERS_SCROLL_KEY, String(pos)); } catch (_e) {}
  }
  ...
  useEffect(() => {
    ...
    let saved = 0;
    try { saved = Number(sessionStorage.getItem(ORDERS_SCROLL_KEY) || 0) || 0; } catch (_e) {}
    ordersScrollPosRef.current = saved;
    if (saved > 0) requestAnimationFrame(() => { mainEl.scrollTop = saved; });
    const onScroll = () => {
      ordersScrollPosRef.current = mainEl.scrollTop;
      try { sessionStorage.setItem(ORDERS_SCROLL_KEY, String(mainEl.scrollTop)); } catch (_e) {}
    };
    mainEl.addEventListener('scroll', onScroll, { passive: true });
    return () => { onScroll(); mainEl.removeEventListener('scroll', onScroll); };
  }, []);
  useEffect(() => {
  ...
  return (
    <div style={styles.viewWrap}>
      ...
      {editingOrder && (
        <div style={styles.modalOverlay} onClick={() => !saving && closeOrderEditor()}>
          <div ref={editOrderModalRef} style={styles.modal} className="alfhd-modal" onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              ...
              <button onClick={closeOrderEditor} style={styles.modalClose}><X size={18} /></button>
            </div>
            ...
            <div style={styles.modalFooter}>
              <button onClick={closeOrderEditor} style={styles.modalCancelBtn}>إلغاء</button>
              <button
...
function AIAssistantView({ currentUser }) {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      try {
        const c = await sbSelectColumns('alfhd_conversations', 'id,customer_name,customer_psid,ai_mode,tab,last_message_time', wsFilter() + '&order=last_message_time.desc&limit=400');
        setConvs(c || []);
  ...
  async function toggleGlobalAI() {
    if (saving) return;
    const scope = settings.runtime_scope || (settings.enabled_globally ? 'selective' : 'off');
    const turnOnAll = scope === 'off';
    const activeN = convs.filter((c) => c.tab !== 'handoff' && c.ai_mode === 'active').length;
    if (!confirm(turnOnAll
      ? 'تشغيل الرد الآلي لكل المحادثات؟'
    ...
    try {
      ...
      const [ctrl] = await sbSelect('ai_control_config', '&id=eq.1');
      setSettings((prev) => ({ ...prev, ...(fresh || {}), runtime_scope: nextScope, runtime_active_count: Number(data.active_count || 0), runtime_eligible_count: Number(data.eligible_count || prev.runtime_eligible_count || 0), booking_commit_enabled: ctrl?.booking_commit_enabled === true }));
      setConvs((prev) => prev.map((c) => ({ ...c, ai_mode: turnOnAll && c.tab !== 'handoff' ? 'active' : (!turnOnAll ? 'paused' : c.ai_mode) })));
  ...
  async function runDiagnostics() {
    ...
    try {
      ...
      // ══ ③ المفتاح العام ══
      { const scope = settings?.runtime_scope || (settings?.enabled_globally ? 'selective' : 'off');
        const activeN = Number(settings?.runtime_active_count || 0);
        add('حالة الرد الآلي', scope === 'off' ? 'bad' : (scope === 'selective' ? 'warn' : 'ok'),
          scope === 'all' ? 'شغال للجميع' : scope === 'selective' ? `تشغيل انتقائي — ${activeN} محادثة` : 'متوقف للجميع',
          scope === 'off' ? 'شغّله للجميع أو فعّل محادثة محددة' : null); }

      // ══ ④ عدد المفعّلة (حِمل النظام) ══
      const activeN = Number(settings?.runtime_active_count || 0);
      const totalN = Number(settings?.runtime_eligible_count || convs.length || 1);
  ...
  async function setConvMode(convId, mode) {
    try {
      ...
      const [fresh] = await sbSelect('ai_settings', '&id=eq.1');
      const [ctrl] = await sbSelect('ai_control_config', '&id=eq.1');
      setSettings((prev) => ({ ...prev, ...(fresh || {}), runtime_scope: data.scope || fresh?.runtime_scope || 'selective', runtime_active_count: Number(data.active_count || 0), runtime_eligible_count: Number(data.eligible_count || prev.runtime_eligible_count || 0), booking_commit_enabled: ctrl?.booking_commit_enabled === true }));
    ...
    <div style={{ padding: 16, maxWidth: 960, margin: '0 auto', direction: 'rtl' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <span style={{
          ...
          background: settings.runtime_scope === 'all' ? 'rgba(34,197,94,0.12)' : settings.runtime_scope === 'selective' ? 'rgba(245,158,11,0.12)' : 'rgba(242,80,80,0.12)',
          color: settings.runtime_scope === 'all' ? '#22C55E' : settings.runtime_scope === 'selective' ? '#F59E0B' : '#F25050',
        }}>{settings.runtime_scope === 'all' ? '● شغال للجميع' : settings.runtime_scope === 'selective' ? `● تشغيل انتقائي — ${Number(settings.runtime_active_count || 0)}` : '● متوقف للجميع'}</span>
      </div>
      ...
      {tab === 'dash' && (<>
        ...
        <div style={box}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={label}>تشغيل الرد الآلي</div>
              <div style={{ fontSize: 12, color: '#8B98A9', lineHeight: 1.6 }}>أخضر: الجميع · برتقالي: محادثات محددة · أحمر: متوقف للجميع</div>
            </div>
            <button
              onClick={toggleGlobalAI}
              disabled={saving}
              style={{ minWidth: 92, height: 34, padding: '0 12px', borderRadius: 18, border: 'none',
                background: settings.runtime_scope === 'all' ? '#22C55E' : settings.runtime_scope === 'selective' ? '#F59E0B' : '#F25050',
                color: '#fff', flexShrink: 0, cursor: saving ? 'wait' : 'pointer', opacity: saving ? .65 : 1, fontWeight: 800, fontFamily: 'inherit', fontSize: 12 }}
            >
              {saving ? '…' : (settings.runtime_scope === 'all' ? 'شغال' : settings.runtime_scope === 'selective' ? `${Number(settings.runtime_active_count || 0)} شغالة` : 'متوقف')}
            </button>
          </div>
          ...
          <div style={{ fontSize: 12, color: '#8FA0B5', marginTop: 10, lineHeight: 1.7 }}>
            تقدر توقف الجميع، أو تخلي النظام انتقائي وتشغّل محادثات محددة فقط. Handoff يبقى حاجزاً إلزامياً دائماً.
          </div>
        </div>
...
function WhChip({ active, label, onClick }) {
  return (
    <button type="button" onClick={onClick} style={{
      display:'flex', alignItems:'center', gap:4, padding:'5px 10px', borderRadius:16, cursor:'pointer',
      ...
      color: active?'#22C55E':'#8FA0B5', fontSize:11.5, fontWeight:700,
    }}>
      {active ? '✓' : ''} {label}
    </button>
  );
}
function WhChipRow({ label, children }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:8 }}>
      <span style={{ fontSize:11, color:'#8FA0B5', minWidth:44 }}>{label}:</span>
      {children}
    </div>
  );
}
// زر بحث دائري عائم — يضل ظاهر دايماً بقسم المنتجات، حتى داخل صفحة التعديل
function WhFloatingSearch({ products, onSelect }) {
  const [open, setOpen] = useState(false);
  const [q, setQ] = useState('');
  const results = q.trim() ? products.filter(p => p.car_name?.includes(q.trim())).slice(0, 30) : [];
  return (
    <>
        ...
        <div onClick={() => { setOpen(false); setQ(''); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 9998, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '10vh 16px' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#141A22', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: 14, width: '100%', maxWidth: 420 }}>
            ...
            <div style={{ marginTop: 10, maxHeight: 320, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              {results.map(p => (
                <button key={p.id} onClick={() => { onSelect(p); setOpen(false); setQ(''); }} style={{ textAlign: 'right', padding: '8px 10px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, color: '#EAF0F7', fontSize: 13, cursor: 'pointer' }}>
                  {p.car_name}
                </button>
              ))}
            </div>
...
// المنتجات
// ══════════════════════════════════════════════════════════════
// صفحة تعديل منتج كاملة — لون بصورته الخاصة + خيارات ركاب/كشنات لكل لون
// ══════════════════════════════════════════════════════════════
function ProductEditPage({ product, onBack, sbI, sbU, sbD, setProducts, allProducts, onSwitchProduct }) {
  const [form, setForm] = useState({
    car_name: product.car_name || '', type: product.type || 'mother_dosah',
    aliases: product.aliases || '', ai_available: product.ai_available !== false,
  });
  ...
  async function loadAll() {
    setLoading(true);
    try {
      const cRows = await sbSelect('wh_product_colors', `&product_id=eq.${product.id}&order=sort_order.asc`);
      const withSeats = await Promise.all((cRows || []).map(async (c) => {
        const seats = await sbSelect('wh_product_seat_options', `&color_id=eq.${c.id}&order=sort_order.asc`);
        return { ...c, seatOptions: seats || [] };
      }));
      setColors(withSeats);
  ...
  async function saveBasic() {
    setSavingBasic(true);
    try {
      const clean = { car_name: form.car_name.trim(), type: form.type, aliases: form.aliases || '', ai_available: form.ai_available };
      await sbU('wh_products', product.id, clean);
      setProducts(prev => prev.map(p => p.id === product.id ? { ...p, ...clean } : p));
    } catch (e) { alert('فشل الحفظ: ' + e.message); }
  ...
  async function addColor() {
    try {
      const r = await sbI('wh_product_colors', { product_id: product.id, color_name: '', origin: 'both', sort_order: colors.length });
      if (r?.[0]) setColors(prev => [...prev, { ...r[0], seatOptions: [] }]);
    } catch (e) { alert('فشل إضافة اللون: ' + e.message); }
  }
  async function updateColorField(colorId, patch) {
    setColors(prev => prev.map(c => c.id === colorId ? { ...c, ...patch } : c));
    try { await sbU('wh_product_colors', colorId, patch); } catch (e) { alert('فشل الحفظ: ' + e.message); }
  }
  async function deleteColor(colorId) {
    if (!confirm('حذف هذا اللون وكل خياراته؟')) return;
    try { await sbD('wh_product_colors', colorId); setColors(prev => prev.filter(c => c.id !== colorId)); }
    catch (e) { alert('فشل الحذف: ' + e.message); }
  }
  function uploadColorImage(colorId, file) {
    if (!file) return;
    ...
    reader.onload = () => {
      ...
      img.onload = async () => {
        try {
          ...
          const filename = `product_color_${colorId}_${Date.now()}.jpg`;
          const up = await fetch(`${SUPABASE_URL}/storage/v1/object/chat-media/${filename}`, {
            method: 'POST', headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'image/jpeg', 'x-upsert': 'true' }, body: blob,
          });
          if (!up.ok) throw new Error('فشل الرفع');
          const url = `${SUPABASE_URL}/storage/v1/object/public/chat-media/${filename}`;
          await updateColorField(colorId, { image_url: url });
        } catch (e) { alert('فشل رفع الصورة: ' + e.message); }
        setUploadingColorId(null);
      };
      img.onerror = () => { setUploadingColorId(null); alert('تعذّر قراءة الصورة'); };
      img.src = reader.result;
    };
    reader.onerror = () => setUploadingColorId(null);
    reader.readAsDataURL(file);
  }
  ...
  async function updateSeatOption(colorId, optId, patch) {
    setColors(prev => prev.map(c => c.id === colorId ? { ...c, seatOptions: c.seatOptions.map(o => o.id === optId ? { ...o, ...patch } : o) } : c));
    try { await sbU('wh_product_seat_options', optId, patch); } catch (e) { alert('فشل الحفظ: ' + e.message); }
  }
  async function deleteSeatOption(colorId, optId) {
    try {
      await sbD('wh_product_seat_options', optId);
      setColors(prev => prev.map(c => c.id === colorId ? { ...c, seatOptions: c.seatOptions.filter(o => o.id !== optId) } : c));
    } catch (e) { alert('فشل الحذف: ' + e.message); }
  }

  const t = WH_PRODUCT_TYPES.find(x => x.id === form.type);
  ...
  return (
    <div style={{ direction: 'rtl' }}>
      ...
      {loading ? (
        <div style={{ padding: 30, textAlign: 'center', color: '#8FA0B5' }}>جارٍ التحميل...</div>
      ) : colors.length === 0 ? (
        <div style={{ padding: 30, textAlign: 'center', color: '#8FA0B5', background: 'rgba(255,255,255,0.02)', borderRadius: 12 }}>
          ما فيه ألوان مضافة بعد — اضغط "إضافة لون" حتى تبدأ
        </div>
      ) : (
            ...
            <div key={c.id} style={{ background: 'linear-gradient(145deg,#141A22,#181F29)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 13, padding: 14 }}>
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 12 }}>
                ...
                <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <input value={c.color_name || ''} onChange={e => updateColorField(c.id, { color_name: e.target.value })} placeholder="اسم اللون: اسود / بيجي" style={whInp} />
                  <select value={c.origin || 'both'} onChange={e => updateColorField(c.id, { origin: e.target.value })} style={whInp}>
                    <option value="both">خليجي وأمريكي</option>
                    <option value="gulf">خليجي فقط</option>
                    <option value="american">أمريكي فقط</option>
                  </select>
                  <input type="number" value={c.model_year_from || ''} onChange={e => updateColorField(c.id, { model_year_from: e.target.value ? Number(e.target.value) : null })} placeholder="من موديل (مثلاً 2016)" style={whInp} />
                  <input type="number" value={c.model_year_to || ''} onChange={e => updateColorField(c.id, { model_year_to: e.target.value ? Number(e.target.value) : null })} placeholder="لين موديل (مثلاً 2020)" style={whInp} />
                </div>
                <button onClick={() => deleteColor(c.id)} style={{ padding: 8, background: 'rgba(242,80,80,0.08)', border: '1px solid rgba(242,80,80,0.18)', borderRadius: 8, color: '#F25050', cursor: 'pointer', flexShrink: 0 }}><Trash2 size={13} /></button>
              </div>

              {/* خيارات الركاب/الكشنات لهذا اللون */}
              <div style={{ marginRight: 84 }}>
                ...
                {c.seatOptions.length === 0 ? (
                  <div style={{ fontSize: 11.5, color: '#8FA0B5', padding: '6px 0' }}>ما فيه خيارات بعد — اضغط "خيار جديد"</div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {c.seatOptions.map(o => {
                      const avail = o.ai_available !== false;
                      return (
                      <div key={o.id} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${avail ? 'rgba(255,255,255,0.07)' : 'rgba(242,80,80,0.15)'}`, borderRadius: 10, padding: 10 }}>
                        {/* الصف الأول: الركاب */}
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8, alignItems: 'center' }}>
                          <select value={o.seats} onChange={e => updateSeatOption(c.id, o.id, { seats: Number(e.target.value) })} style={{ ...whInp, width: 90, padding: '6px 8px', fontSize: 12 }}>
                            <option value={5}>5 راكب</option>
                        ...
                        <WhChipRow label="الكشنات">
                          <WhChip active={o.kashat_type === 'متصل'} label="متصل" onClick={() => updateSeatOption(c.id, o.id, { kashat_type: o.kashat_type === 'متصل' ? null : 'متصل' })} />
                          <WhChip active={o.kashat_type === 'منفصل'} label="منفصل" onClick={() => updateSeatOption(c.id, o.id, { kashat_type: o.kashat_type === 'منفصل' ? null : 'منفصل' })} />
                        </WhChipRow>
                        {/* المنشأ — علّم على اللي موجود، لو الاثنين ينعتبر "الكل" */}
                        <WhChipRow label="المنشأ">
                          <WhChip active={(o.origin||'both')==='both'||(o.origin||'both')==='gulf'} label="خليجي" onClick={() => {
                            const cur=o.origin||'both'; const gulfOn=cur==='both'||cur==='gulf'; const amerOn=cur==='both'||cur==='american';
                            const ng=!gulfOn; updateSeatOption(c.id,o.id,{origin: ng&&amerOn?'both':ng?'gulf':amerOn?'american':'both'});
                          }} />
                          <WhChip active={(o.origin||'both')==='both'||(o.origin||'both')==='american'} label="أمريكي" onClick={() => {
                            const cur=o.origin||'both'; const gulfOn=cur==='both'||cur==='gulf'; const amerOn=cur==='both'||cur==='american';
                            const na=!amerOn; updateSeatOption(c.id,o.id,{origin: gulfOn&&na?'both':na?'american':gulfOn?'gulf':'both'});
                          }} />
                        </WhChipRow>
                        {/* الدفع — فورويل/توويل/هايبرد */}
                        <WhChipRow label="الدفع">
                          <WhChip active={(o.drivetrain||'all')==='all'||(o.drivetrain||'all')==='fourwd'} label="فورويل" onClick={() => {
                            const cur=o.drivetrain||'all'; if(cur==='hybrid'){updateSeatOption(c.id,o.id,{drivetrain:'fourwd'});return;}
                            const fOn=cur==='all'||cur==='fourwd'; const tOn=cur==='all'||cur==='twowd'; const nf=!fOn;
                            updateSeatOption(c.id,o.id,{drivetrain: nf&&tOn?'all':nf?'fourwd':tOn?'twowd':'all'});
                          }} />
                          <WhChip active={(o.drivetrain||'all')==='all'||(o.drivetrain||'all')==='twowd'} label="توويل" onClick={() => {
                            const cur=o.drivetrain||'all'; if(cur==='hybrid'){updateSeatOption(c.id,o.id,{drivetrain:'twowd'});return;}
                            const fOn=cur==='all'||cur==='fourwd'; const tOn=cur==='all'||cur==='twowd'; const nt=!tOn;
                            updateSeatOption(c.id,o.id,{drivetrain: fOn&&nt?'all':nt?'twowd':fOn?'fourwd':'all'});
                          }} />
                          <WhChip active={(o.drivetrain||'all')==='hybrid'} label="هايبرد" onClick={() => updateSeatOption(c.id, o.id, { drivetrain: o.drivetrain==='hybrid'?'all':'hybrid' })} />
                        </WhChipRow>
                        {/* الصف الثاني: السعر + الكمية + الموقع */}
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                          <input type="text" inputMode="numeric" defaultValue={o.price_iqd || ''}
                            onBlur={e => { const v = formatPriceInput(e.target.value); e.target.value = v; updateSeatOption(c.id, o.id, { price_iqd: v ? Number(v) : 0 }); }}
                            placeholder="السعر (مثلاً 110)" style={{ ...whInp, width: 130, padding: '6px 8px', fontSize: 12 }} />
                          <input type="number" value={o.quantity || ''} onChange={e => updateSeatOption(c.id, o.id, { quantity: e.target.value ? Number(e.target.value) : 0 })} placeholder="عدد القطع" style={{ ...whInp, width: 100, padding: '6px 8px', fontSize: 12 }} />
                          <input type="text" value={o.location || ''} onChange={e => updateSeatOption(c.id, o.id, { location: e.target.value })} placeholder="الموقع: السرة 1 الرف 2" style={{ ...whInp, flex: 1, minWidth: 140, padding: '6px 8px', fontSize: 12 }} />
                        </div>
                        {/* الصف الثالث: توفر الذكاء + حذف */}
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
                          <button onClick={() => updateSeatOption(c.id, o.id, { ai_available: !avail })}
                            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 7, border: `1px solid ${avail ? 'rgba(34,197,94,0.35)' : 'rgba(242,80,80,0.3)'}`, background: avail ? 'rgba(34,197,94,0.14)' : 'rgba(242,80,80,0.12)', color: avail ? '#22C55E' : '#F25050', fontSize: 11.5, fontWeight: 800, cursor: 'pointer' }}>
                            <span style={{ width: 8, height: 8, borderRadius: '50%', background: avail ? '#22C55E' : '#F25050', display: 'inline-block' }} />
                            {avail ? 'متوفر للذكاء' : 'غير متوفر للذكاء'}
                          </button>
                          <button onClick={() => deleteSeatOption(c.id, o.id)} style={{ padding: 7, background: 'rgba(242,80,80,0.08)', border: '1px solid rgba(242,80,80,0.18)', borderRadius: 7, color: '#F25050', cursor: 'pointer', flexShrink: 0 }}><Trash2 size={12} /></button>
                        </div>
                      </div>
                    );})}
                  </div>
...
function WhProducts({ products, setProducts, cars, setCars, sbI, sbU, sbD }) {
  const [editingFull,setEditingFull]=useState(null); // منتج مفتوح بصفحة التعديل الكاملة
  const [search,setSearch]=useState('');
  const [type,setType]=useState('all');
  const [seatFilter,setSeatFilter]=useState('all'); // 'all' | 5 | 7 — يبين بس تحت قسم أم الدوسة
  ...
  const [seatGroups,setSeatGroups]=useState({}); // product_id -> اكبر عدد ركاب مسجل بخياراته
  useEffect(() => {
    const dosahIds = products.filter(p => p.type === 'mother_dosah').map(p => p.id);
    if (!dosahIds.length) { setSeatGroups({}); return; }
    (async () => {
      try {
        const colorsRes = await sbSelect('wh_product_colors', `&product_id=in.(${dosahIds.join(',')})`);
        if (!colorsRes.length) { setSeatGroups({}); return; }
        const colorIds = colorsRes.map(c => c.id);
        const optsRes = await sbSelect('wh_product_seat_options', `&color_id=in.(${colorIds.join(',')})`);
        const colorToProduct = {}; colorsRes.forEach(c => { colorToProduct[c.id] = c.product_id; });
        const map = {};
        optsRes.forEach(o => { const pid = colorToProduct[o.color_id]; if (!pid) return; const cur = map[pid] || 0; if (o.seats > cur) map[pid] = o.seats; });
        setSeatGroups(map);
  ...
  if (editingFull) {
    return <ProductEditPage product={editingFull} onBack={() => { setEditingFull(null); setTimeout(() => { const mainEl=document.querySelector('.alfhd-main-area'); if(mainEl) mainEl.scrollTop=scrollPosRef.current; else window.scrollTo(0, scrollPosRef.current); }, 0); }} sbI={sbI} sbU={sbU} sbD={sbD} setProducts={setProducts} allProducts={products} onSwitchProduct={(p) => setEditingFull(p)} />;
  }
  return (
    <div>
      ...
      <div style={{display:'flex',gap: 8,marginBottom:type==='mother_dosah'?8:12,flexWrap:'wrap',alignItems:'center'}}>
      ...
      <div style={{background:'linear-gradient(145deg,#141A22,#181F29)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:13,overflow:'hidden'}}>
        {filtered.length===0?<div style={{padding:40,textAlign:'center',color:'#8FA0B5'}}>لا توجد منتجات</div>: (() => {
          const renderCard = (p,i,arr) => {
            ...
            const avail=p.ai_available!==false;
            return(
              <div key={p.id} style={{padding: '14px 16px',borderBottom:i<arr.length-1?'1px solid rgba(255,255,255,0.06)':'none'}}>
                ...
                <div style={{display:'flex',gap: 8,flexWrap:'wrap',marginBottom:10}}>
                  <span style={{fontSize: 12,fontWeight:700,color:'#EAF0F7',background:'rgba(255,255,255,0.05)',padding:'4px 10px',borderRadius:8}}>
                    {p.price_iqd>0 ? whFmt(p.price_iqd) : 'السعر حسب الطلب'}
                  </span>
                  {p.location&&<span style={{fontSize: 12,color:'#8FA0B5',background:'rgba(255,255,255,0.03)',padding:'4px 10px',borderRadius:8}}>📍 {p.location}</span>}
                </div>
          ...
          if (type !== 'mother_dosah') return filtered.map((p,i)=>renderCard(p,i,filtered));
          if (seatFilter === 5) { const g=filtered.filter(p=>seatGroups[p.id]!==7); return g.length? g.map((p,i)=>renderCard(p,i,g)) : <div style={{padding:30,textAlign:'center',color:'#8FA0B5'}}>ما فيه منتجات خمسة راكب</div>; }
          if (seatFilter === 7) { const g=filtered.filter(p=>seatGroups[p.id]===7); return g.length? g.map((p,i)=>renderCard(p,i,g)) : <div style={{padding:30,textAlign:'center',color:'#8FA0B5'}}>ما فيه منتجات سبعة راكب</div>; }
          ...
          return (
            <>
              {fiveGroup.length>0 && sectionHeader('🚗 خمسة راكب', fiveGroup.length)}
              {fiveGroup.map((p,i)=>renderCard(p,i,fiveGroup))}
              {sevenGroup.length>0 && sectionHeader('🚐 سبعة راكب', sevenGroup.length)}
              {sevenGroup.map((p,i)=>renderCard(p,i,sevenGroup))}
            </>
          );
        })()}
      </div>
...
const styles = {
  ...
  modalBody: { padding: '16px 16px', display: 'flex', flexDirection: 'column', gap: 12, overflowY: 'auto', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', flex: 1, minHeight: 0 },
  ...
  modalFooter: { zIndex: 4, display: 'flex', gap: 8, padding: '12px 16px', borderTop: `1px solid ${TB}`, background: '#141A22', boxShadow: '0 -4px 16px rgba(0,0,0,.22)', flexShrink: 0 },
