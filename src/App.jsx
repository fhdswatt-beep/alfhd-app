    ...
    {selectedConv ? ( <>
      ...
      <style>{`
        @media (max-width: 860px) {
          ...
          /* منطقة الرسائل: تملأ المساحة بين الترويسة والشريط، تتمرّر داخلياً */
          ...
          /* شريط الكتابة: ثابت أسفل، جزء من العمود، ما ينزاح */
          ...
          /* إخفاء النافبار السفلي لما محادثة مفتوحة (يعطي مساحة كاملة) */
          body:has(.alfhd-conv-detail-active-mobile) nav.alfhd-no-print { display: none !important; }
          body:has(.alfhd-conv-detail-active-mobile) header.alfhd-no-print { display: none !important; }
...
export default function AlFhdApp() {
  ...
  return (
    ...
    <>
      <GlobalStyles />
      {/* ── جرس الإشعارات (أعلى يسار — بمحاذاة الترويسة) ── */}
      <div style={{ position: 'fixed', top: 8, left: 12, zIndex: 110 }} className="alfhd-no-print alfhd-notif-bell">
        <button
          onClick={() => {
...
function AIAssistantView({ currentUser }) {
  ...
  // إضافة تصحيح لرد غلط → ينضاف كمثال تدريبي مُلزم
  async function saveCorrection(log) {
    ...
    const updated = (settings.training_examples || '') + block;
    try {
      await sbUpdate('ai_settings', 1, { training_examples: updated, updated_at: new Date().toISOString() });
      setSettings({ ...settings, training_examples: updated });
      setCorrecting(null); setCorrectionText('');
      alert('انضاف التصحيح للتدريب — الذكاء راح يلتزم بيه بالردود الجاية');
  ...
  return (
    <div style={{ padding: 16, maxWidth: 960, margin: '0 auto', direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', gap: 7, marginBottom: 14, overflowX: 'auto', paddingBottom: 4 }}>
        <button onClick={() => setTab('dash')} style={tabBtn('dash')}>لوحة عامة</button>
        <button onClick={() => setTab('training')} style={tabBtn('training')}>التدريب والتعليمات</button>
        <button onClick={() => setTab('log')} style={tabBtn('log')}>سجل الردود ({total})</button>
        <button onClick={() => setTab('convs')} style={tabBtn('convs')}>المحادثات ({convs.length})</button>
        <button onClick={() => setTab('products')} style={tabBtn('products')}>توفر المنتجات</button>
        {drafts.length > 0 && <button onClick={() => setTab('drafts')} style={tabBtn('drafts')}>مسودات ({drafts.length})</button>}
      </div>
      ...
      {/* ═══════ التدريب والتعليمات ═══════ */}
      {tab === 'training' && (<>
        {/* إعدادات API — تحط أي مفتاح وموديل من أي شركة */}
              ...
              {correcting === l.id && (
                <div style={{ marginTop: 10, background: 'rgba(242,80,80,0.05)', border: '1px solid rgba(242,80,80,0.2)', borderRadius: 10, padding: 11 }}>
                  ...
                  <div style={{ display: 'flex', gap: 7, marginTop: 8 }}>
                    ...
                    <button onClick={() => { setCorrecting(null); setCorrectionText(''); }} style={{ padding: '7px 14px', borderRadius: 9, border: '1px solid #222C42', background: 'transparent', color: '#8B98A9', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>إلغاء</button>
                  </div>
                </div>
              )}
            </div>
