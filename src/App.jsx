...
function ConversationsView({ conversations, pages, orders, setOrders, setConversations, pendingOpenConvId, clearPendingOpenConvId, onCreateOrderFromConv, onOpenOrderDetails, currentUser }) {
 ...
 // ── فحص خلفي: أي محادثة فيها رقم وعنوان بلا طلب مثبّت ──
 const [readyConvIds, setReadyConvIds] = useState(new Set());
 useEffect(() => {
  let cancelled = false;
  (async () => {
   // المرشحون: بلا طلب، وفيهم نشاط
   const cands = conversations.filter((c) => !c.orderId && c.tab !== 'pinned').slice(0, 120);
   if (!cands.length) { setReadyConvIds(new Set()); return; }
   const found = new Set();
   // ندرسهم على دفعات حتى ما نثقل الشبكة
   for (let i = 0; i < cands.length; i += 12) {
    ...
    try {
     const rows = await sbSelect('alfhd_messages',
      `&conversation_id=in.(${ids})&select=conversation_id,content&order=created_at.desc&limit=400`);
     const byConv = {};
    ...
    } catch (_e) { /* تجاهل الدفعة */ }
   }
   if (!cancelled) setReadyConvIds(found);
  })();
  return () => { cancelled = true; };
  // eslint-disable-next-line react-hooks/exhaustive-deps
 }, [conversations.length, conversations.map((c) => c.orderId).join(',')]);
 ...
 const tabLabels = { normal: 'اعتيادية', pinned: 'مثبّت بها طلب', handoff: 'ذكاء اصطناعي', ready: 'بيانات كاملة' };
 const tabIcons  = { normal: MessageSquare, pinned: Pin, ready: AlertCircle, handoff: Bot };
   ...
   >
    ...
    {(currentUser?.role === 'admin' || (currentUser?.permissions || []).includes('ai_manage')) && (
     <button
      ...
      style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '8px',
...
function AIAssistantView({ currentUser }) {
  ...
  async function runDiagnostics() {
    ...
    const r = { steps: [] };
    const add = (name, ok, hint) => r.steps.push({ name, ok, hint });
    try {
      // 1) المفتاح العام
      add('المفتاح العام للذكاء', !!settings?.enabled_globally,
        'روح لتبويب "نظرة عامة" وشغّل المفتاح العام');

      // 2) مفتاح API
      const hasKey = !!(settings?.api_key || '').trim();
      add('مفتاح API محفوظ بالموقع', hasKey,
        'إذا فاضي، لازم يكون OPENAI_API_KEY مضبوط بأسرار الدالة بـ Supabase');

      // 3) محادثات مفعّلة
      const activeCount = convs.filter((c) => c.ai_mode === 'active').length;
      add(`محادثات مفعّلة (${activeCount})`, activeCount > 0,
        'الذكاء ما يرد إلا على محادثة تفعّلها بالزر الأخضر داخل المحادثة');
      ...
      try {
        ...
        fnMsg = d.error ? String(d.error).slice(0, 90) : (fnOk ? '' : 'ردت بلا نص');
      } catch (e) { fnMsg = String(e).slice(0, 90); }
      add('دالة ai-auto-reply تشتغل', fnOk,
        fnMsg || 'تأكد من نشر الدالة وإن JWT verify مطفي');

      // 5) التريغر — نفحص وصول ردود حديثة
      const recent = logs.filter((l) => {
        const t = new Date(l.created_at).getTime();
        return Date.now() - t < 48 * 3600 * 1000;
      }).length;
      add(`ردود بآخر 48 ساعة (${recent})`, recent > 0,
        'إذا صفر مع وجود رسائل واردة: التريغر مو منصّب — شغّل ملف 08_ai_trigger بـ SQL');
            ...
            <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {diag.steps.map((st, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start',
                  ...
                  background: st.ok ? 'rgba(34,197,94,0.07)' : 'rgba(242,80,80,0.07)',
                  border: `1px solid ${st.ok ? 'rgba(34,197,94,0.22)' : 'rgba(242,80,80,0.25)'}` }}>
                  <span style={{ fontSize: 15, lineHeight: 1.3 }}>{st.ok ? '✓' : '✗'}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 700, color: st.ok ? '#22C55E' : '#F25050' }}>{st.name}</div>
                    {!st.ok && st.hint && (
                      <div style={{ fontSize: 12, color: '#8B98A9', marginTop: 4, lineHeight: 1.6 }}>{st.hint}</div>
                    )}
                  </div>
                </div>
              ))}
              {diag.steps.every((s2) => s2.ok) && (
                <div style={{ fontSize: 12, color: '#22C55E', fontWeight: 700, padding: '4px 2px' }}>
                  كل الحلقات سليمة — الذكاء جاهز يرد على المحادثات المفعّلة
                </div>
              )}
            </div>
          )}
