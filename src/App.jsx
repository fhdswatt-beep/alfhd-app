       ...
       return (
        <button
         onClick={async () => {
           ...
           const msg = allOn
             ? `إيقاف الرد الآلي بكل المحادثات (${conversations.length})؟`
             : `تشغيل الرد الآلي بكل المحادثات (${conversations.length})؟\nحالياً مفعّل بـ${activeN} فقط.`;
         ...
         title={`الرد الآلي مفعّل بـ${activeN} من ${conversations.length} محادثة — اضغط ${allOn ? 'لإيقاف' : 'لتشغيل'} الكل`}
         style={{ position: 'relative', width: 44, height: 44, borderRadius: 12, flexShrink: 0,
        ...
        >
         <Bot size={19} />
         {/* العدّاد: يوضّح كم محادثة مفعّلة فعلاً — بدل الحالة الثنائية المضلّلة */}
         <span style={{ position: 'absolute', bottom: -4, insetInlineStart: -4, minWidth: 20, height: 17, padding: '0 4px',
...
export default function AlFhdApp() {
  ...
  useEffect(() => {
    ...
    const start = () => {
      if (!convTimer) convTimer = setInterval(tickConv, 2000);   // القائمة: كل ثانيتين
      if (!fbTimer) fbTimer = setInterval(tickFb, 4000);          // جلب فيسبوك: كل 4 ثوانٍ
    };
    const stop = () => {
