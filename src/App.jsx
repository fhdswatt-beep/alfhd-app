 ...
 const MenuOverlay = () => !menuOpen ? null : (
  <div onClick={() => setMenuOpen(false)}
   ...
   <div onClick={(e) => e.stopPropagation()}
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '18px 16px', borderBottom: '1px solid #222C42' }}>
     <FahdLogo size={32} />
     <div style={{ flex: 1 }}>
      <div style={{ fontSize: 15, fontWeight: 800, color: '#F5F5F5' }}>AlFhd</div>
      <div style={{ fontSize: 11, color: '#546880' }}>{currentUser.name}</div>
     </div>
     <button onClick={() => setMenuOpen(false)} style={{ padding: 7, background: 'transparent', border: 'none', color: '#8B98A9', cursor: 'pointer', display: 'flex' }}><X size={19} /></button>
    </div>
     ...
     {visibleMenu.map((item) => {
      ...
      return (
       <button key={item.id} onClick={() => { setActiveView(item.id); setMenuOpen(false); }}
        style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '13px 12px', marginBottom: 6, borderRadius: 12, border: 'none', cursor: 'pointer', fontFamily: "'Cairo', sans-serif", textAlign: 'right',
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
         ...
         <div style={{ fontSize: 11, color: '#546880', marginTop: 1 }}>{item.desc}</div>
        </div>
       </button>
      );
     })}
    ...
    <div style={{ padding: 12, borderTop: '1px solid #222C42' }}>
     <button onClick={onLogout} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '11px', borderRadius: 11, border: 'none', background: 'rgba(242,80,80,0.10)', color: '#F25050', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: "'Cairo', sans-serif" }}>
...
function ConversationsView({ conversations, pages, orders, setConversations, pendingOpenConvId, clearPendingOpenConvId, onCreateOrderFromConv, onOpenOrderDetails, currentUser }) {
 ...
 async function maybeCreateOrderFromAI(convId, messages) {
  const conv = conversations.find((c) => c.id === convId);
  if (!conv || conv.orderId) return; // عنده طلب أصلاً — لا نكرر
  ...
  const grab = (label) => { const m = t.match(new RegExp(label + '\\s*:?\\s*(.+)')); return m ? m[1].trim() : ''; };
 ...
 return ( /* ─── حاوية المحادثات ─── */
  <div style={{ display: 'flex', overflow: 'hidden', direction: 'rtl', background: '#0E1621', width: '100%', height: '100%', }} className="alfhd-conv-fullscreen">
   ...
   >
    ...
    {/* ── زر عام: تشغيل/إيقاف الرد الآلي لكل المحادثات ── */}
    {(currentUser?.role === 'admin' || (currentUser?.permissions || []).includes('ai_manage')) && (
     <button
      onClick={async () => {
        const anyActive = conversations.some((c) => c.ai_mode === 'active');
        const next = anyActive ? 'paused' : 'active';
        if (!confirm(anyActive ? 'إيقاف الرد الآلي بكل المحادثات؟' : 'تشغيل الرد الآلي بكل المحادثات؟')) return;
      ...
      style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, width: '100%', padding: '9px',
       background: conversations.some((c) => c.ai_mode === 'active') ? 'rgba(34,197,94,0.10)' : 'rgba(242,80,80,0.10)',
       border: 'none', borderBottom: `1px solid ${TG_BDR}`,
       color: conversations.some((c) => c.ai_mode === 'active') ? '#22C55E' : '#F25050',
       fontSize: 12, fontWeight: 800, cursor: 'pointer', fontFamily: "'Cairo', sans-serif" }}
     >
      <Bot size={14} />
      {globalAiBusy ? 'جاري التغيير...' : (conversations.some((c) => c.ai_mode === 'active') ? 'الرد الآلي شغّال — اضغط لإيقاف الكل' : 'الرد الآلي متوقف — اضغط لتشغيل الكل')}
     </button>
    )}
    {/* ── قائمة المحادثات ── */}
    ...
    {selectedConv ? ( <>
       ...
       {(currentUser?.role === 'admin' || (currentUser?.permissions || []).includes('ai_manage')) && (
        <button
         onClick={() => {
           ...
           // الحفظ بالخلفية — إذا فشل نرجع الحالة
           sbUpdate('alfhd_conversations', selectedConv.id, { ai_mode: next }).catch((e) => {
             setSelectedConv((c) => (c ? { ...c, ai_mode: cur } : c));
             setConversations?.((prev) => prev.map((c) => (c.id === selectedConv.id ? { ...c, ai_mode: cur } : c)));
             setAiToast('فشل التغيير: ' + e.message);
             setTimeout(() => setAiToast(null), 2500);
           });
         }}
         title="تشغيل / إيقاف الرد الآلي بهذي المحادثة"
         style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 10px', border: 'none', borderRadius: 20, fontSize: 12, fontWeight: 700, flexShrink: 0, cursor: 'pointer', transition: 'all .15s',
      ...
      {linkedOrder && ( <div style={{ background: 'rgba(42,171,238,0.06)', borderBottom: `1px solid rgba(42,171,238,0.15)`, padding: orderCardOpen ? '10px 16px' : '7px 16px', flexShrink: 0, }} className="alfhd-linked-order">
        <button
         ...
         style={{ display: 'flex', alignItems: 'center', gap: 7, width: '100%', background: 'transparent', border: 'none', padding: 0, fontSize: 11.5, fontWeight: 700, color: TG_BLUE, marginBottom: orderCardOpen ? 8 : 0, cursor: 'pointer' }}
        >
         ...
         <span style={{ marginRight: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
          {!orderCardOpen && <span style={{ fontSize: 11.5, color: TG_BLUE, fontWeight: 700 }}>{linkedOrder.total.toLocaleString()} د.ع</span>}
      ...
      >
       {loadingMsgs ? ( <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1 }}>
        ...
        </div> ) : messages.length === 0 ? ( <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: 14, color: TG_DIM }}>
      ...
      {/* ── رسالة تأكيد فورية ── */}
      {aiToast && (
       <div style={{ position: 'absolute', bottom: 78, left: '50%', transform: 'translateX(-50%)', zIndex: 40, padding: '9px 18px', borderRadius: 22, background: 'rgba(19,27,38,0.96)', border: `1px solid ${TG_BDR}`, color: TG_TEXT, fontSize: 12.5, fontWeight: 700, boxShadow: '0 6px 20px rgba(0,0,0,0.4)', whiteSpace: 'nowrap' }}>
        {aiToast}
       </div>
      )}
      {/* ── شريط حالة الذكاء الصناعي ── */}
      {selectedConv.ai_mode === 'active' && (
       <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: 'rgba(34,197,94,0.07)', borderTop: '1px solid rgba(34,197,94,0.15)', flexShrink: 0 }}>
      ...
      <style>{`
        @media (max-width: 860px) {
          ...
          .alfhd-ai-btn-label { display: none !important; }
...
function OrdersView({ orders, pages, setOrders, conversations, setConversations, onViewConversation, pendingNewOrderFromConv, clearPendingNewOrderFromConv, currentUser, onContactCustomer, pendingOpenOrderId, clearPendingOpenOrderId, warehouseProducts = [] }) {
  ...
  const stats = useMemo(() => {
    let scoped = selectedPage === 'all' ? visibleOrders : visibleOrders.filter((o) => o.pageId === selectedPage);
    // في قسم التوصيل: العدادات تتغير مع فلتر التاريخ المختار
    if (section === 'delivery') {
      scoped = scoped.filter((o) => dateInRange(o.date || o.createdAt, datePreset, customMonth, customYear));
    }
          ...
          {['all', 'pending', 'delivered', 'returned'].map((s) => {
            ...
            return (
              <button key={s} onClick={() => setStatusFilter(s)} style={{ ...styles.chip, ...(statusFilter === s ? styles.chipActive : {}) }}>
                ...
                <span style={{ ...styles.convTabCount, ...(statusFilter === s ? styles.convTabCountActive : {}), marginRight: 5 }}>{cnt}</span>
              </button>
            );
          })}
        </div>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {prepSelected.length > 0 && (
              ...
              <button onClick={printSelectedPrepSheets}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 10, border: '1px solid rgba(34,197,94,0.4)', cursor: 'pointer',
...
function AIAssistantView({ currentUser }) {
  ...
  const box = { background: '#131B26', border: '1px solid #222C42', borderRadius: 14, padding: 16, marginBottom: 14 };
  ...
  const inp = { width: '100%', background: '#0E1621', border: '1px solid #222C42', borderRadius: 10, color: '#F5F5F5', padding: '9px 12px', fontSize: 13, fontFamily: 'inherit', direction: 'rtl' };
  const tabBtn = (id) => ({ padding: '8px 14px', borderRadius: 10, border: 'none', background: tab === id ? '#2AABEE' : '#131B26', color: tab === id ? '#fff' : '#8B98A9', fontWeight: 700, fontSize: 12.5, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'inherit' });
  const stat = (v, l, c) => (
    <div style={{ flex: '1 1 100px', background: '#131B26', border: '1px solid #222C42', borderRadius: 12, padding: '12px 10px', textAlign: 'center' }}>
      <div style={{ fontSize: 21, fontWeight: 800, color: c }}>{v}</div>
      <div style={{ fontSize: 11.5, color: '#8B98A9', marginTop: 2 }}>{l}</div>
    </div>
  );
  ...
  return (
    <div style={{ padding: 16, maxWidth: 960, margin: '0 auto', direction: 'rtl' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 14, flexWrap: 'wrap' }}>
        ...
        <span style={{
          marginRight: 'auto', fontSize: 11.5, fontWeight: 700, padding: '4px 10px', borderRadius: 20,
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, overflowX: 'auto', paddingBottom: 6, WebkitOverflowScrolling: 'touch' }} className="alfhd-ai-tabs">
        {[
          { id: 'dash',     label: 'نظرة عامة', icon: BarChart3, badge: null },
          { id: 'convs',    label: 'المحادثات', icon: MessageSquare, badge: convs.length },
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 13px', borderRadius: 11,
              ...
              {t.badge != null && t.badge > 0 && (
                <span style={{ background: on ? 'rgba(42,171,238,0.3)' : 'rgba(255,255,255,0.08)',
                  borderRadius: 20, padding: '1px 6px', fontSize: 10.5, fontWeight: 800,
      ...
      {tab === 'dash' && (<>
        ...
        {(noPriceProducts.length > 0 || noImageProducts.length > 0) && (
          <div style={{ ...box, background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.22)' }}>
            ...
            {noPriceProducts.length > 0 && (
              <div style={{ fontSize: 12.5, color: '#8B98A9', marginBottom: 6 }}>
                <b style={{ color: '#F25050' }}>{noPriceProducts.length} منتج بدون سعر</b> — الذكاء ما يقدر يبيعهم، يصعّدهم لموظف:
                <div style={{ fontSize: 11.5, color: '#546880', marginTop: 3 }}>{noPriceProducts.slice(0, 8).map((p) => p.car_name).join(' · ')}{noPriceProducts.length > 8 ? ' ...' : ''}</div>
              </div>
            )}
        ...
        <div style={box}>
          ...
          <button onClick={save} disabled={saving} style={{ marginTop: 12, padding: '9px 20px', borderRadius: 10, border: 'none', background: '#2AABEE', color: '#fff', fontWeight: 800, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
        ...
        <div style={box}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 11, color: '#546880' }}>الذكاء يتذكر كل الرسائل — مثل زبون حقيقي</span>
            {testChat.length > 0 && (
              <button onClick={resetTest} style={{ marginRight: 'auto', padding: '5px 11px', borderRadius: 8, border: '1px solid #222C42', background: 'transparent', color: '#F25050', fontSize: 11.5, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {/* شاشة المحادثة */}
          <div style={{ background: '#0E1621', border: '1px solid #222C42', borderRadius: 12, padding: 12, minHeight: 120, maxHeight: 420, overflowY: 'auto', marginBottom: 10 }}>
            {testChat.length === 0 && (
              <div style={{ color: '#546880', fontSize: 12.5, textAlign: 'center', padding: '22px 10px', lineHeight: 1.8 }}>
                ابدأ محادثة تجريبية<br />
                <span style={{ fontSize: 11.5 }}>جرب: "مرحبا" ثم "ماليبو موجود؟" ثم "شكد سعرها؟"</span>
              </div>
            )}
            {testChat.map((m, i) => (
              <div key={i} style={{ marginBottom: 11, display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-start' : 'flex-end' }}>
                <div style={{ maxWidth: '85%', padding: '9px 12px', borderRadius: 13,
                  ...
                  <div style={{ fontSize: 10.5, color: '#546880', marginBottom: 3, fontWeight: 700 }}>
                  ...
                  {m.image && (
                    <div style={{ marginTop: 7 }}>
                      <div style={{ fontSize: 10.5, color: '#22C55E', marginBottom: 4, fontWeight: 700 }}>✓ يرسل صورة المنتج</div>
                  ...
                  {m.role === 'assistant' && Array.isArray(m.tools) && m.tools.length > 0 && (
                    <details style={{ marginTop: 7 }}>
                      <summary style={{ fontSize: 10.5, color: '#2AABEE', cursor: 'pointer', fontWeight: 700 }}>
                        على شنو اعتمد؟ ({m.tools.length}) {m.valid === false && <span style={{ color: '#F25050' }}>· ما اجتاز التحقق</span>}
                      </summary>
                      <pre style={{ fontSize: 10, color: '#8B98A9', background: '#131B26', padding: 8, borderRadius: 7, overflowX: 'auto', marginTop: 6, direction: 'ltr', textAlign: 'left', maxHeight: 200 }}>
                  ...
                  {m.role === 'assistant' && i === testChat.length - 1 && !testing && (
                    <button onClick={regenerateLast}
                      style={{ marginTop: 8, padding: '5px 11px', borderRadius: 8, border: '1px solid rgba(42,171,238,0.3)', background: 'rgba(42,171,238,0.08)', color: '#2AABEE', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            {testing && <div style={{ fontSize: 12, color: '#546880', textAlign: 'center', padding: 8 }}>جاري الرد...</div>}
          </div>

          {/* حقل الكتابة */}
          <div style={{ display: 'flex', gap: 7 }}>
            <input style={{ ...inp, flex: 1 }} placeholder="اكتب مثل الزبون..." value={testQ}
        ...
        <div style={box}>
          <div style={{ ...label, marginBottom: 10 }}>آخر الحالات التي صُعّدت لموظف</div>
          {logs.filter((l) => l.escalated).slice(0, 5).length === 0
            ...
            : logs.filter((l) => l.escalated).slice(0, 5).map((l) => (
              <div key={l.id} style={{ borderRight: '3px solid #F59E0B', paddingRight: 10, marginBottom: 10 }}>
                <div style={{ fontSize: 12.5, color: '#F5F5F5' }}>{l.customer_message || '(بدون نص)'}</div>
                <div style={{ fontSize: 11.5, color: '#F59E0B', marginTop: 2 }}>{l.escalation_reason}</div>
              </div>
            ))}
          <button onClick={() => setTab('log')} style={{ marginTop: 8, padding: '7px 14px', borderRadius: 9, border: '1px solid #222C42', background: 'transparent', color: '#2AABEE', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
            عرض السجل الكامل
          </button>
        </div>
      </>)}
      ...
      {tab === 'settings' && (<>
        <div style={{ ...box, background: 'rgba(139,92,246,0.05)', border: '1px solid rgba(139,92,246,0.2)' }}>
          ...
          <div style={{ fontSize: 11.5, color: '#8B98A9', lineHeight: 1.7, marginBottom: 12 }}>
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <input type="password" style={inp} placeholder="sk-... (اتركه فاضي للإبقاء على الحالي)"
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <input style={inp} placeholder="gpt-4o-mini / gpt-4o / deepseek-chat ..."
              value={settings.model || ''} onChange={(e) => setSettings({ ...settings, model: e.target.value })} />
            <div style={{ fontSize: 10.5, color: '#546880', marginTop: 5, lineHeight: 1.6 }}>
          ...
          <div>
            ...
            <input style={inp} placeholder="اتركه فاضي لـ OpenAI"
          ...
          <button onClick={save} disabled={saving} style={{ marginTop: 12, padding: '9px 20px', borderRadius: 10, border: 'none', background: '#A78BFA', color: '#fff', fontWeight: 800, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      {tab === 'training' && (<>
        ...
        {/* ملخص حالة التدريب */}
        <div style={{ ...box, padding: '11px 13px', display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 10.5, color: '#546880', fontWeight: 700 }}>مصطلحات القاموس</div>
            <div style={{ fontSize: 17, fontWeight: 800, color: '#22C55E' }}>
              {((settings.dialect_glossary || '').match(/^-\s*.+=/gm) || []).length}
            </div>
          </div>
          <div>
            <div style={{ fontSize: 10.5, color: '#546880', fontWeight: 700 }}>تصحيحاتك المحفوظة</div>
            <div style={{ fontSize: 17, fontWeight: 800, color: correctionCount >= 35 ? '#F59E0B' : '#2AABEE' }}>
              {correctionCount}<span style={{ fontSize: 11, color: '#546880' }}> / 40</span>
            </div>
          </div>
          {correctionCount >= 35 && (
            <div style={{ fontSize: 11, color: '#F59E0B', flex: 1, minWidth: 160, lineHeight: 1.6 }}>
        ...
        <div style={{ ...box, background: 'rgba(34,197,94,0.04)', border: '1px solid rgba(34,197,94,0.2)' }}>
          ...
          <div style={{ fontSize: 11.5, color: '#8B98A9', lineHeight: 1.7, marginBottom: 9 }}>
          ...
          <button onClick={save} disabled={saving} style={{ marginTop: 10, padding: '8px 18px', borderRadius: 10, border: 'none', background: '#22C55E', color: '#fff', fontWeight: 800, fontSize: 12.5, cursor: 'pointer', fontFamily: 'inherit' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginTop: 9, flexWrap: 'wrap' }}>
            <button onClick={enhanceInstructions} disabled={enhanceBusy || !(settings.system_prompt || '').trim()}
              style={{ padding: '8px 16px', borderRadius: 10, border: 'none', cursor: enhanceBusy ? 'wait' : 'pointer',
                background: 'linear-gradient(135deg,#A78BFA,#8B5CF6)', color: '#fff', fontSize: 12.5, fontWeight: 800, fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}>
              <Sparkles size={14} /> {enhanceBusy ? 'جاري التحسين...' : 'حسّن كلامي (اكتب بلهجتك والذكاء يحوّلها احترافية)'}
            </button>
          </div>
          {/* معاينة النص المحسّن */}
          {enhancePreview && (
            <div style={{ marginTop: 12, padding: 14, borderRadius: 12, background: 'rgba(167,139,250,0.06)', border: '1px solid rgba(167,139,250,0.25)' }}>
              <div style={{ fontSize: 12.5, fontWeight: 800, color: '#A78BFA', marginBottom: 8 }}>النسخة المحسّنة (احترافية وصارمة):</div>
              <div style={{ fontSize: 13, color: '#E7ECF3', lineHeight: 1.8, whiteSpace: 'pre-wrap', maxHeight: 300, overflowY: 'auto', background: '#0E1621', padding: 11, borderRadius: 9 }}>{enhancePreview}</div>
        ...
        <div style={box}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, color: '#546880' }}>{(settings.training_examples || '').length} حرف</span>
          </div>
          ...
          <div style={{ fontSize: 11.5, color: '#546880', marginTop: 7, lineHeight: 1.7 }}>
            ملاحظة: التصحيحات اللي تضيفها من سجل الردود تنحفظ هنا تلقائياً كتصحيحات مُلزمة.
          </div>
        </div>

        <button onClick={save} disabled={saving} style={{ padding: '11px 24px', borderRadius: 12, border: 'none', background: '#2AABEE', color: '#fff', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      {tab === 'log' && (<>
        <div style={{ ...box, padding: 12 }}>
          <input style={{ ...inp, marginBottom: 9 }} placeholder="ابحث بالسجل — نص الزبون، الرد، سبب التصعيد، اسم منتج..." value={logSearch} onChange={(e) => setLogSearch(e.target.value)} />
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', alignItems: 'center' }}>
            {[['all', `الكل (${total})`], ['ok', `ناجحة (${okCount})`], ['escalated', `مُصعّدة (${escCount})`]].map(([id, lbl]) => (
              <button key={id} onClick={() => setLogFilter(id)} style={{ padding: '6px 12px', borderRadius: 9, border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', background: logFilter === id ? '#2AABEE' : '#0E1621', color: logFilter === id ? '#fff' : '#8B98A9' }}>{lbl}</button>
            ))}
            <button onClick={loadLogs} style={{ marginRight: 'auto', padding: '6px 12px', borderRadius: 9, border: '1px solid #222C42', background: 'transparent', color: '#8B98A9', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontFamily: 'inherit' }}>
        ...
        {filteredLogs.map((l) => {
          ...
          return (
            <div key={l.id} style={{ ...box, marginBottom: 10, padding: 13, borderRight: `3px solid ${l.escalated ? '#F59E0B' : '#22C55E'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 7, gap: 8, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 11, color: '#546880' }}>{new Date(l.created_at).toLocaleString('ar-IQ')}</span>
                <span style={{ fontSize: 11, fontWeight: 700, color: l.escalated ? '#F59E0B' : '#22C55E' }}>
                  {l.escalated ? '⚠ صُعّدت لموظف' : '✓ رد آلي'}
                </span>
              </div>
              {l.customer_message && <div style={{ fontSize: 12.5, color: '#8B98A9', marginBottom: 5 }}><b style={{ color: '#546880' }}>الزبون:</b> {l.customer_message}</div>}
              {l.reply_text && <div style={{ fontSize: 12.5, color: '#F5F5F5', marginBottom: 5, lineHeight: 1.7 }}><b style={{ color: '#546880' }}>الرد:</b> {l.reply_text}</div>}
              {l.escalation_reason && <div style={{ fontSize: 12, color: '#F59E0B', marginBottom: 5 }}><b>سبب التصعيد:</b> {l.escalation_reason}</div>}

              <div style={{ display: 'flex', gap: 7, marginTop: 9, flexWrap: 'wrap' }}>
                <button onClick={() => setExpandedLog(isOpen ? null : l.id)} style={{ padding: '6px 11px', borderRadius: 8, border: '1px solid #222C42', background: 'transparent', color: '#2AABEE', fontSize: 11.5, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                  {isOpen ? 'إخفاء المصدر' : `على شنو اعتمد؟ (${tools.length} استعلام)`}
                </button>
                <button onClick={() => { setCorrecting(l.id); setCorrectionText(''); }} style={{ padding: '6px 11px', borderRadius: 8, border: '1px solid rgba(242,80,80,0.25)', background: 'rgba(242,80,80,0.06)', color: '#F25050', fontSize: 11.5, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                  هذا غلط — صحّحه
                </button>
              </div>

              {isOpen && (
                <div style={{ marginTop: 10, background: '#0E1621', border: '1px solid #222C42', borderRadius: 10, padding: 11 }}>
                  <div style={{ fontSize: 11.5, fontWeight: 700, color: '#546880', marginBottom: 7 }}>الاستعلامات التي بُني عليها الرد (مصدر الحقيقة)</div>
                  {tools.length === 0 && <div style={{ fontSize: 12, color: '#F25050' }}>⚠ ما استعلم على شي — أي رقم بالرد غير موثق</div>}
                  {tools.map((t, i) => (
                    <pre key={i} style={{ fontSize: 11, color: '#8B98A9', background: '#131B26', padding: 9, borderRadius: 8, overflowX: 'auto', margin: '0 0 7px', direction: 'ltr', textAlign: 'left', fontFamily: 'monospace' }}>
                      {JSON.stringify(t, null, 1)}
                    </pre>
                  ))}
                  <div style={{ fontSize: 11.5, color: l.validation_passed ? '#22C55E' : '#F25050', fontWeight: 700 }}>
              ...
              {correcting === l.id && (
                <div style={{ marginTop: 10, background: 'rgba(242,80,80,0.05)', border: '1px solid rgba(242,80,80,0.2)', borderRadius: 10, padding: 11 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#F25050', marginBottom: 7 }}>شنو كان لازم يجاوب بالضبط؟</div>
                  ...
                  <div style={{ marginTop: 9, paddingTop: 9, borderTop: '1px dashed rgba(255,255,255,0.08)' }}>
                    <div style={{ fontSize: 11.5, fontWeight: 700, color: '#22C55E', marginBottom: 5 }}>
                    ...
                    <input value={glossaryTerm} onChange={(e) => setGlossaryTerm(e.target.value)}
                      ...
                      style={{ width: '100%', background: '#0E1621', border: '1px solid rgba(34,197,94,0.25)', borderRadius: 9, color: '#F5F5F5', padding: '8px 11px', fontSize: 12, fontFamily: 'inherit', boxSizing: 'border-box' }} />
                  </div>
                  <div style={{ display: 'flex', gap: 7, marginTop: 8 }}>
                    <button onClick={() => saveCorrection(l)} style={{ padding: '7px 14px', borderRadius: 9, border: 'none', background: '#22C55E', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>احفظ التصحيح</button>
                    <button onClick={() => { setCorrecting(null); setCorrectionText(''); setGlossaryTerm(''); }} style={{ padding: '7px 14px', borderRadius: 9, border: '1px solid #222C42', background: 'transparent', color: '#8B98A9', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>إلغاء</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {tab === 'convs' && (<>
        <div style={{ ...box, padding: 12 }}>
          <div style={{ fontSize: 12, color: '#8B98A9', lineHeight: 1.7 }}>
            تحكم بوضع الذكاء لكل محادثة: <b style={{ color: '#22C55E' }}>فعّال</b> (يرد تلقائي) · <b style={{ color: '#F59E0B' }}>مراجعة</b> (يسوي مسودة وتوافق) · <b style={{ color: '#94A3B8' }}>متوقف</b> (ترد انت يدوياً)
          </div>
          <div style={{ display: 'flex', gap: 7, marginTop: 10, flexWrap: 'wrap' }}>
            <button onClick={async () => { if (!confirm('تفعيل الذكاء بكل المحادثات؟')) return; for (const c of convs) await setConvMode(c.id, 'active'); }} style={{ padding: '7px 13px', borderRadius: 9, border: 'none', background: 'rgba(34,197,94,0.12)', color: '#22C55E', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>تفعيل الكل</button>
            <button onClick={async () => { if (!confirm('إيقاف الذكاء بكل المحادثات؟')) return; for (const c of convs) await setConvMode(c.id, 'paused'); }} style={{ padding: '7px 13px', borderRadius: 9, border: 'none', background: 'rgba(148,163,184,0.12)', color: '#94A3B8', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>إيقاف الكل</button>
          </div>
        </div>
        <div style={{ ...box, padding: 0, overflow: 'hidden' }}>
          {convs.slice(0, 200).map((c, i) => {
            const mode = c.ai_mode || 'paused';
            const name = c.customer_name || (c.customer_psid || '').replace('wa_', '') || 'زبون';
            return (
              <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '10px 13px', borderBottom: i < Math.min(convs.length, 200) - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                <div style={{ flex: 1, minWidth: 0, fontSize: 12.5, color: '#F5F5F5', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</div>
                <div style={{ display: 'flex', gap: 4 }}>
                  {[['active', 'فعّال', '#22C55E'], ['review', 'مراجعة', '#F59E0B'], ['paused', 'متوقف', '#94A3B8']].map(([m, lbl, col]) => (
                    <button key={m} onClick={() => setConvMode(c.id, m)} style={{ padding: '5px 9px', borderRadius: 7, border: 'none', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', background: mode === m ? `${col}22` : 'transparent', color: mode === m ? col : '#546880' }}>{lbl}</button>
                  ))}
                </div>
              </div>
            );
      ...
      {tab === 'products' && (<>
        <div style={{ ...box, padding: 12 }}>
          <input style={inp} placeholder="ابحث عن منتج..." value={prodSearch} onChange={(e) => setProdSearch(e.target.value)} />
          <div style={{ fontSize: 11.5, color: '#8B98A9', marginTop: 9, lineHeight: 1.7 }}>
          ...
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #222C42' }}>
            <div style={{ fontSize: 12.5, fontWeight: 700, color: '#F5F5F5', marginBottom: 9 }}>صورة موحّدة لكل نوع</div>
            <div style={{ fontSize: 11, color: '#546880', marginBottom: 10, lineHeight: 1.6 }}>ارفع صورة وحدة تنطبق على كل منتجات النوع فوراً. مفيدة الآن — لاحقاً تقدر تغيّر صورة أي منتج لحاله.</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {WH_PRODUCT_TYPES.map((t) => {
                ...
                return (
                  <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 11px', background: '#0E1621', border: '1px solid #222C42', borderRadius: 11 }}>
                    {sample?.image_url
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: withImg === typeProds.length ? '#22C55E' : '#F59E0B' }}>
                        {withImg}/{typeProds.length} عندهم صورة
                      </div>
                    </div>
                    <label style={{ padding: '7px 13px', background: 'rgba(42,171,238,0.1)', border: '1px solid rgba(42,171,238,0.3)', borderRadius: 9, color: '#2AABEE', fontSize: 12, fontWeight: 700, cursor: bulkImgBusy ? 'wait' : 'pointer', flexShrink: 0 }}>
          ...
          {prodSearch.trim() && filteredProducts.length > 0 && (
            <div style={{ marginTop: 11, paddingTop: 11, borderTop: '1px solid #222C42', display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
              <span style={{ fontSize: 12, color: '#8B98A9', fontWeight: 700 }}>تعديل سعر الـ{filteredProducts.length} منتج الظاهرين:</span>
              <input id="bulkPrice" type="number" placeholder="مبلغ" style={{ ...inp, width: 90, padding: '6px 9px' }} />
              <button onClick={async () => {
                ...
                alert('تم');
              }} style={{ padding: '6px 13px', borderRadius: 9, border: 'none', background: '#2AABEE', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                تعيين موحّد
              </button>
              <button onClick={async () => {
                ...
                alert('تم');
              }} style={{ padding: '6px 13px', borderRadius: 9, border: '1px solid #222C42', background: 'transparent', color: '#22C55E', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                نسبة %
              </button>
            </div>
          )}
        ...
        <div style={{ ...box, padding: 0, overflow: 'hidden' }}>
          {filteredProducts.map((p, i) => {
            ...
            return (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', borderBottom: i < filteredProducts.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                {p.image_url
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#546880', display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <input
                      ...
                      placeholder="السعر"
                      style={{ width: 62, background: !p.price_iqd ? 'rgba(242,80,80,0.1)' : '#0E1621', border: `1px solid ${!p.price_iqd ? 'rgba(242,80,80,0.3)' : '#222C42'}`, borderRadius: 6, color: !p.price_iqd ? '#F25050' : '#F5F5F5', padding: '2px 6px', fontSize: 11, fontFamily: 'inherit', textAlign: 'center' }}
                    />
                    {!p.image_url && <span style={{ color: '#F59E0B' }}>⚠ بلا صورة</span>}
                  </div>
                </div>
                <button onClick={() => toggleProduct(p)} style={{ padding: '6px 12px', borderRadius: 20, border: `1px solid ${avail ? 'rgba(34,197,94,0.3)' : 'rgba(242,80,80,0.25)'}`, background: avail ? 'rgba(34,197,94,0.12)' : 'rgba(242,80,80,0.08)', color: avail ? '#22C55E' : '#F25050', fontSize: 11.5, fontWeight: 700, cursor: 'pointer', flexShrink: 0, fontFamily: 'inherit' }}>
        ...
        <div style={box}>
          ...
          {drafts.map((d) => (
            <div key={d.id} style={{ borderBottom: '1px solid #222C42', padding: '12px 0' }}>
              <div style={{ fontSize: 11, color: '#546880', marginBottom: 5 }}>{new Date(d.created_at).toLocaleString('ar-IQ')}</div>
              <div style={{ fontSize: 12.5, color: '#F5F5F5', lineHeight: 1.7, marginBottom: 9 }}>{d.draft_text}</div>
              <div style={{ display: 'flex', gap: 7 }}>
                <button onClick={() => approveDraft(d, true)} style={{ padding: '7px 14px', borderRadius: 9, border: 'none', background: '#22C55E', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>موافق</button>
                <button onClick={() => approveDraft(d, false)} style={{ padding: '7px 14px', borderRadius: 9, border: '1px solid rgba(242,80,80,0.25)', background: 'transparent', color: '#F25050', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>رفض</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {modal&&(
        <WhModal title={editing?'تعديل منتج':'إضافة منتج'} onClose={()=>setModal(false)}>
          ...
          <WhField label="أسماء بديلة (اللهجة العراقية)">
            ...
            <div style={{fontSize:11,color:'#546880',marginTop:5,lineHeight:1.6}}>الأسماء اللي الزبون يستخدمها لنفس السيارة — افصلها بفاصلة. الذكاء يبحث بيها مثل الاسم الأصلي.</div>
          </WhField>
            ...
            <div style={{display:'flex',gap:6}}>
              {[['both','خليجي وأمريكي','#2AABEE'],['gulf','خليجي فقط','#8B5CF6'],['american','أمريكي فقط','#F59E0B']].map(([v,lbl,col])=>(
                <button key={v} type="button" onClick={()=>setForm(f=>({...f,origin:v}))}
                  style={{flex:1,padding:'8px 6px',borderRadius:9,border:'none',fontSize:11.5,fontWeight:700,cursor:'pointer',fontFamily:'inherit',
                    background:(form.origin||'both')===v?`${col}22`:'#0E1621',color:(form.origin||'both')===v?col:'#546880'}}>{lbl}</button>
              ))}
            </div>
          </WhField>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:7}}>
              {(form.colors||[]).map((c,ci)=>(
                <div key={ci} style={{display:'flex',gap:6,alignItems:'center'}}>
                  ...
                  <select value={c.scope||'all'} onChange={e=>setForm(f=>{const a=[...(f.colors||[])];a[ci]={...a[ci],scope:e.target.value};return{...f,colors:a};})}
                  ...
                  <button type="button" onClick={()=>setForm(f=>({...f,colors:(f.colors||[]).filter((_,i)=>i!==ci)}))}
                    style={{padding:7,background:'rgba(242,80,80,0.08)',border:'none',borderRadius:8,color:'#F25050',cursor:'pointer',display:'flex'}}><Trash2 size={13}/></button>
                </div>
              ))}
              ...
              <div style={{fontSize:11,color:'#546880',lineHeight:1.6}}>إذا ما ضفت أي لون، الذكاء يعتبر المتوفر أسود فقط.</div>
            </div>
          </WhField>
...
const styles = {
  ...
  convTabCount: { background: 'rgba(255,255,255,0.08)', borderRadius: 20, padding: '1px 6px', fontSize: 11.5, fontWeight: 700, whiteSpace: 'nowrap', minWidth: 18, textAlign: 'center', flexShrink: 0 },
  ...
  msgBubbleIn: { background: '#1C2A38', border: 'none', borderRadius: '18px 18px 18px 5px', padding: '9px 13px', fontSize: 14, color: '#F5F5F5', maxWidth: '76%', lineHeight: 1.55, display: 'flex', flexDirection: 'column', gap: 2, boxShadow: '0 1px 3px rgba(0,0,0,0.18)', overflowWrap: 'anywhere', wordBreak: 'break-word', minWidth: 0, willChange: 'transform, opacity' },
  msgBubbleOut: { background: 'linear-gradient(135deg,#2E6199,#2B5278)', border: 'none', borderRadius: '18px 18px 5px 18px', padding: '9px 13px', fontSize: 14, color: '#fff', maxWidth: '76%', lineHeight: 1.55, display: 'flex', flexDirection: 'column', gap: 2, boxShadow: '0 1px 3px rgba(43,82,120,0.35)', overflowWrap: 'anywhere', wordBreak: 'break-word', minWidth: 0, willChange: 'transform, opacity' },
  ...
  msgTime: { fontSize: 10.5, color: 'rgba(255,255,255,0.5)', alignSelf: 'flex-end', fontWeight: 500, marginTop: 1 },
  ...
  sectionTab: { display: 'flex', alignItems: 'center', gap: 7, padding: '7px 13px', background: 'transparent', border: 'none', borderRadius: 20, color: TDM, fontSize: 12.5, fontWeight: 600, transition: 'all 0.12s ease', whiteSpace: 'nowrap', flexShrink: 0 },
